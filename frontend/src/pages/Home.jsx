...
import { Link } from "react-router-dom"; // ✨ Adicionamos useLocation para saber a rota atual
...
// A Função agora aceita o parâmetro de ordenação
const agruparVagas = (listaVagas, ordenarPorMaisVistos = false) => {
  const agrupadas = [];

  // 1. Faz o agrupamento normal
  ...
  // 2. Calcula o total de cliques para CADA grupo
  agrupadas.forEach(grupo => {
    const cliquesFilhas = grupo.similares.reduce((soma, filha) => soma + (filha.cliques || 0), 0);
    grupo.totalCliques = (grupo.cliques || 0) + cliquesFilhas;
  });

  // 3. Ordenação
  if (ordenarPorMaisVistos) {
    agrupadas.sort((a, b) => b.totalCliques - a.totalCliques);
  } else {
    agrupadas.sort((a, b) => new Date(b.dia) - new Date(a.dia));
  }
...
const Home = () => {
  const { vagas, loading, erro, ordemAtual } = useContext(ContextApi);
  ...
  const vagasProcessadas = useMemo(() => {
    ...
    const isTelaMaisVistos = ordemAtual === 'most_viewed'; 
    return agruparVagas(vagas, isTelaMaisVistos);
  }, [vagas, ordemAtual]);
        ...
        {loading ? (
          ...
          <div id="jobs-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vagasProcessadas.map((vaga, index) => {
              ...
              // Já passamos a usar o totalCliques que a função agruparVagas gerou!
              return (
                <React.Fragment key={vaga.id}>
                  <Card 
                    ...
                    cliques={vaga.totalCliques} // 🔥 AQUI ENTRA A VARIÁVEL DA SOMA QUE VEIO DO AGRUPADOR 🔥
                    dia={vaga.dia} 
                  ...
                  return (
                    <div key={v.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-primary/50 bg-white dark:bg-gray-800 shadow-sm transition-all relative overflow-hidden gap-3">
                      ...
                      <Link 
                        ...
                        onClick={() => registrarCliqueModal(v.id)} 
